...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isButton()) {
    if (interaction.customId === "formulario") {
      ...
      const primeiraPergunta = new Discord.ActionRowBuilder().addComponents(
        pergunta1
      );
      const segundaPergunta = new Discord.ActionRowBuilder().addComponents(
        pergunta2
      );
      const terceiraPergunta = new Discord.ActionRowBuilder().addComponents(
        pergunta3
      );
    ...
    if (interaction.customId === "modal") {
      ...
      let embed = new Discord.EmbedBuilder()
        ...
        .setDescription(
          `O usuário ${interaction.user}, enviou o formulário abaixo:`
        )
        .addFields(
          {
      ...
      interaction.reply({
        content: `Olá **${interaction.user}**, seu formulário foi enviado com sucesso!`,
        ephemeral: true,
      });
      await interaction.guild.channels.cache
        .get(settings.logsChannelId)
        .send({ embeds: [embed] });
    }
  }
});
